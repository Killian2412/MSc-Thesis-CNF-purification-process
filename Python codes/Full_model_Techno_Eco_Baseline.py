...
def full_calculation(parameters):
    ...
    print('Q cold cooler is', Q_cold_cooler)
    print('T water out cooler', T_water_out_cooler)
    print('T water in cooler', T_water_in_cooler)
    ...
    Ni_molarflow_CSTR_out = sim.STRM_CISOLID_Get_MoleFlowPerCompound("CSTROUT", "NI")
    ...
    print('dust waste flow in tonnes per year', dust_waste_flow)
    ...
    print('waste-6', Massflow_acid_waste_condensor)
    ...
    print('Steam-1 massflow is', pump_massflow)
    print('Steam-2 massflow is', massflow_steam_required_CSTR)
    print('Steam-4 massflow is', massflow_steam_air_heater + massflow_steam_slurry_heater)
    print('Steam-5 massflow is', massflow_steam_slurry_heater)
    print('Steam-7 massflow is', massflow_steam_air_heater)
    ...
    print('water yearly consumption in m^3/year', water_consumption_yearly)
    print('Water-2 flow in kg/hr', massflow_water_cooler)
    print('Water-7 flow in kg/hr', Massflow_water_air_cooler)
    print('Water-4 flow in kg/hr is', pump_massflow)
    ...
    print("cstr size is", CSTR_volume)
    print('solids cooler size is', Area_cooler)
    print('separator size is', volumeflow_total_separator_in_Ls)
    print('hydrocyclone size is', volumeflow_total_hydrocyclone_in_Ls)
    print('filter1 size is', filter1_area)
    print('washer size is', volumeflow_total_washer_in_Ls)
    print('filter2 size is', filter2_area)
    print('slurry heater size is', Area_slurry_heater)
    print('dryer size is', Area_dryer)
    print('steam boiler size is', steam_boiler_rating)
    print('water pump size is', pump_massflow_Ls)
    print('air filter size is', flow_dry_airNm3pers)
    print('air heater size is', Area_air_heater)
    print('air cooler size is', Area_air_cooler)
...
sorted_items = sorted(isbl_cost_dictionary.items(), key=lambda item: item[1], reverse=True) #With ISBL cost
#sorted_items = sorted(equipment_cost_dictionary.items(), key=lambda item: item[1], reverse=True) #Without ISBL cost
...
format = mtick.FuncFormatter(lambda y, p: f'{y/1e6:,.0f}')
...
sorted_extracted_costs = sorted(extracted_costs_dict.items(), key=lambda item: item[1], reverse=True)
sorted_extracted_dict = dict(sorted_extracted_costs)
...
axis2.yaxis.set_major_formatter(format)
...
axis3.set_xlabel('')
axis3.yaxis.set_major_formatter(format)
