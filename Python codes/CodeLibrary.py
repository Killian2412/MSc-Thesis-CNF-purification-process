...
#This just shortens the path you need to call for Streams and Blocks
    ...
    @property
    def REACTION(self):
        return self.AspenSimulation.Tree.Elements("Data").Elements("Reactions")
...
###PAGE 3       Kinetics
    ...
    #def BLK_CISTR_Set_Reaction_HasChildren(self, Blockname:str, Children1):
        #self.BLK.Elements(Blockname).Elements("Input").Elements("RXN_ID").Children = Children1
    #def BLK_CISTR_Set_Reaction_Selection(self, Blockname:str, Select_Reaction: Literal["R-1"]):
        #self.BLK.Elements(Blockname).Elements("Input").Elements("RXN_ID").Elements("#0").Value = Select_Reaction
    #def BLK_CISTR_Set_Reaction_Select(self, Blockname:str, React_Select: Literal["YES", "NO"]):
        #self.BLK.Elements(Blockname).Elements("Input").Elements("REORDER").Value = React_Select
    #def BLK_Get_Options(self, Blockname:str):
        #opties = self.BLK.Elements(Blockname).Elements("Input").Elements("RXN_ID").Value
        #return opties
...
#Specifications: PAGE 1
    ...
    def STRM_Set_TotalFlowRate(self, Streamname, TotalFlowRate):
        self.STRM.Elements(Streamname).Elements("Input").Elements("TOTFLOW").Elements("MIXED").Value = TotalFlowRate
    def STRM_Set_TotalFlowBasis(self, Streamname, TotalFlowBasis): #removed Compoundname. WOULD HAVE TO ADD A LITERAL: WITH THE OPTIONS
    ...
    def STRM_Set_Basis(self, Streamname:str, Pannekoek: Literal["MASS-FLOW", "MOLE-FLOW", "STDVOL-FLOW", "MASS-FRAC", "MOLE-FRAC", "STDVOL-FRAC", "MASS-CONC", "MOLE-CONC"]):
        self.STRM.Elements(Streamname).Elements("Input").Elements("BASIS").Elements("MIXED").Value = Pannekoek
#Page 2    CI Solid
    ###MISSING
#PAGE 3     NC Solid
    ##MISSING
...
#Outputs for Streams
    ...
    def STRM_Get_PropertySet2(self, Streamname):
        return self.STRM.Elements(Streamname).Elements("Output").Elements("PPOPSET").Value #This one is actually correct, one above this is bullshit
    ...
    def STRM_Get_TotalFlow(self, Streamname):
        return self.STRM.Elements(Streamname).Elements("Output").Elements("MASSFLMX").Elements("$TOTAL").Value
...
#CISOLID INPUT
    ...
    def STRM_CISOLID_GET_ME_ALL_INPUTS_BACK(self, Streamname: str) -> Dict[str, Union[str, float, int]]:
        """Retrieves all the Inputs and returns Dictionary with Values
        ...
        Args:
            Blockname: String which gives the name of Block.
        """
        FlashtypeChoice = self.STRM.Elements(Streamname).Elements("Input").Elements("MIXED_SPEC").Elements("CISOLID").Value
        Temp = self.STRM.Elements(Streamname).Elements("Input").Elements("TEMP").Elements("CISOLID").Value
        Pressure = self.STRM.Elements(Streamname).Elements("Input").Elements("PRES").Elements("CISOLID").Value
        VaporFraction = self.STRM.Elements(Streamname).Elements("Input").Elements("VFRAC").Elements("CISOLID").Value
        TotalFlowRate = self.STRM.Elements(Streamname).Elements("Input").Elements("CISOLID").Value

        CompoundNameList = []
        TotalFlowBasisList = []
        ComponentFlowRateList = []
        CompoundNameNode = self.STRM.Elements(Streamname).Elements("Input").Elements("FLOW").Elements("CISOLID").Elements
        for CompoundName in CompoundNameNode:
            CompoundNameList.append(CompoundName)
            TotalFlowBasisList.append(self.STRM.Elements(Streamname).Elements("Input").Elements("FLOW").Elements("CISOLID").Elements(CompoundName).Value)
            ComponentFlowRateList.append(self.STRM.Elements(Streamname).Elements("Input").Elements("FLOW").Elements("CISOLID").Elements(CompoundName).Value)

        CalculateStreamPropertiesOption = self.STRM.Elements(Streamname).Elements("Input").Elements("FL_OPTION").Elements("CISOLID").Value
        RemoveComponentOption = self.STRM.Elements(Streamname).Elements("Input").Elements("AUTO_COMPS").Value
        ComponentTolerance = self.STRM.Elements(Streamname).Elements("Input").Elements("AUTO_COMPS_T").Value
        ChooseAdditionalOptions = self.STRM.Elements(Streamname).Elements("Input").Elements("EO_COMPS").Value
        SolutionMethod = self.STRM.Elements(Streamname).Elements("Input").Elements("SOL_METHOD").Value
    ...
    def STRM_CISOLID_SET_ALL_INPUTS(self, Streamname:str, Dictionary: Dict[str, Union[str,float,int]]) -> None:
        """Takes Dictionary with Values set the ones which are given in Aspen.
        ...
        #Specifications: PAGE 1
        self.STRM.Elements(Streamname).Elements("Input").Elements("MIXED_SPEC").Elements("CISOLID").Value = Dictionary.get("FlashtypeChoice")
        self.STRM.Elements(Streamname).Elements("Input").Elements("TEMP").Elements("CISOLID").Value = Dictionary.get("Temp")
        self.STRM.Elements(Streamname).Elements("Input").Elements("PRES").Elements("CISOLID").Value = Dictionary.get("Pressure")
        self.STRM.Elements(Streamname).Elements("Input").Elements("VFRAC").Elements("CISOLID").Value = Dictionary.get("VaporFraction")
        self.STRM.Elements(Streamname).Elements("Input").Elements("TOTFLOW").Value = Dictionary.get("TotalFlowRate")
        ...
        i = 0
        for Compoundname in CompoundNameList:
            self.STRM.Elements(Streamname).Elements("Input").Elements("FLOW").Elements("CISOLID").Elements(Compoundname).Value = TotalFlowBasisList[i]
            self.STRM.Elements(Streamname).Elements("Input").Elements("FLOW").Elements("CISOLID").Elements(Compoundname).Value = ComponentFlowRateList[i]
            i = i + 1
        i = 0
        #Page 2    CI Solid
        #PAGE 3     NC Solid
        #PAGE 4     Flash Option
        self.STRM.Elements(Streamname).Elements("Input").Elements("FL_OPTION").Elements("CISOLID").Value = Dictionary.get("CalculateStreamPropertiesOption")
        #PAGE 5 EO OPTIONS
        self.STRM.Elements(Streamname).Elements("Input").Elements("AUTO_COMPS").Value = Dictionary.get("RemoveComponentOption")
        self.STRM.Elements(Streamname).Elements("Input").Elements("AUTO_COMPS_T").Value = Dictionary.get("ComponentTolerance")
        self.STRM.Elements(Streamname).Elements("Input").Elements("EO_COMPS").Value = Dictionary.get("ChooseAdditionalOptions")
...
#### INPUT FOR CISOLID STREAMS

#Specifications: PAGE 1
    def STRM_CISOLID_Set_FlashTypeOption(self, Streamname:str, FlashtypeChoice: Literal["TP", "TV", "PV"]) -> None:        #   This choses the Inputs: can take TP, TV, PV
        self.STRM.Elements(Streamname).Elements("Input").Elements("MIXED_SPEC").Elements("CISOLID").Value = FlashtypeChoice
    def STRM_CISOLID_Set_Temperature(self, Streamname, Temp):
        self.STRM.Elements(Streamname).Elements("Input").Elements("TEMP").Elements("CISOLID").Value = Temp
    def STRM_CISOLID_Set_Pressure(self, Streamname, Pressure):
        self.STRM.Elements(Streamname).Elements("Input").Elements("PRES").Elements("CISOLID").Value = Pressure
    def STRM_CISOLID_Set_VaporFraction(self, Streamname, VaporFraction):
        self.STRM.Elements(Streamname).Elements("Input").Elements("VFRAC").Elements("CISOLID").Value = VaporFraction
    def STRM_CISOLID_Set_TotalFlowRate(self, Streamname, TotalFlowRate):
        self.STRM.Elements(Streamname).Elements("Input").Elements("TOTFLOW").Elements("CISOLID").Value = TotalFlowRate
    def STRM_CISOLID_Set_TotalFlowBasis(self, Streamname, TotalFlowBasis, Compoundname):
        self.STRM.Elements(Streamname).Elements("Input").Elements("FLOW").Elements("CISOLID").Elements(Compoundname).Value = TotalFlowBasis
    def STRM_CISOLID_Set_ComponentFlowRate(self, Streamname, ComponentFlowRate, Compoundname):
        self.STRM.Elements(Streamname).Elements("Input").Elements("FLOW").Elements("CISOLID").Elements(Compoundname).Value = ComponentFlowRate
    ...
    def REACTION_POWERLAW_Set_preexpfactor(self, reactionname, preexp):
        self.REACTION.Elements("Reactions").Elements("reactionname").Elements("Input").Elements("PRE_EXP").Elements("1").Value = preexp

    def BLK_BATCH_Set_Stopcriteria(self, Blockname, Criteria):
        self.BLK.Elements(Blockname).Elements("Input").Elements("Value").Elements("1").Value = Criteria

    def BLK_BATCH_Set_Cycletime(self, Blockname, Time):
        self.BLK.Elements(Blockname).Elements("Input").Elements("CYCLE_TIME").Value = Time

    def BLK_BATCH_Set_Calctime(self, Blockname, calctime):
        self.BLK.Elements(Blockname).Elements("Input").Elements("MAX_TIME").Value = calctime

#CISOLID OUTPUTS


    def STRM_CISOLID_GET_OUTPUTS(self, Streamname: str) -> Dict[str, Union[str, float, int]]:
        """Retrieves all Output variables for given Block and returns Dictionary of Values

            Args:
                Blockname: String which gives the name of Block.
        """
        Compoundlist = self.STRM.Elements(Streamname).Elements("Output").Elements("MOLEFRAC").Elements("CISOLID").Elements
        CompoundNameList = []
        MoleFlowList = []
        MassFlowList = []
        MoleFracList = []
        ...
        for compound in Compoundlist:
            ...
            CompoundNameList.append(Compoundname)
            MoleFlowList.append(
                self.STRM.Elements(Streamname).Elements("Output").Elements("STR_MAIN").Elements("MOLEFLOW").Elements(
                    "CISOLID").Elements(Compoundname).Value)
            MassFlowList.append(
                self.STRM.Elements(Streamname).Elements("Output").Elements("STR_MAIN").Elements("MASSFLOW").Elements(
                    "CISOLID").Elements(Compoundname).Value)
            MoleFracList.append(
                self.STRM.Elements(Streamname).Elements("Output").Elements("MOLEFRAC").Elements("CISOLID").Elements(
                    Compoundname).Value)
            MassFracList.append(
                self.STRM.Elements(Streamname).Elements("Output").Elements("MASSFRAC").Elements("CISOLID").Elements(
                    Compoundname).Value)
            #LiquidConcentrationList.append(
            #    self.STRM.Elements(Streamname).Elements("Output").Elements("X").Elements(Compoundname).Value)
            #VaporConcentrationList.append(
            #    self.STRM.Elements(Streamname).Elements("Output").Elements("Y").Elements(Compoundname).Value)

        Source = self.STRM.Elements(Streamname).Elements("Output").Elements("STR_MAIN").Elements("SOURCE").Value
        Destination = self.STRM.Elements(Streamname).Elements("Output").Elements("STR_MAIN").Elements(
            "DESTINATION").Value
        Phase = self.STRM.Elements(Streamname).Elements("Output").Elements("STR_MAIN").Elements("COMPTYPE").Value
        PropertySet = self.STRM.Elements(Streamname).Elements("Output").Elements("STR_MAIN").Elements("PPOPSET").Value
        VolumeFlow = self.STRM.Elements(Streamname).Elements("Output").Elements("STR_MAIN").Elements(
            "VOLFLMX").Elements("CISOLID").Value
    ...
    def STRM_CISOLID_Get_MoleFlowPerCompound(self, Streamname, Compoundname):
        return self.STRM.Elements(Streamname).Elements("Output").Elements("STR_MAIN").Elements("MOLEFLOW").Elements("CISOLID").Elements(Compoundname).Value

    def STRM_CISOLID_Get_MassFlowPerCompound(self, Streamname, Compoundname):
        return self.STRM.Elements(Streamname).Elements("Output").Elements("STR_MAIN").Elements("MASSFLOW").Elements("CISOLID").Elements(Compoundname).Value

    def STRM_CISOLID_Get_VolumeFlow(self, Streamname):
        return self.STRM.Elements(Streamname).Elements("Output").Elements("STR_MAIN").Elements("VOLFLMX").Elements("CISOLID").Value

    def STRM_CISOLID_Get_Temperature(self, Streamname):
        return self.STRM.Elements(Streamname).Elements("Output").Elements("STR_MAIN").Elements("TEMP").Elements("CISOLID").Value

    def STRM_CISOLID_Get_Pressure(self, Streamname):
        return self.STRM.Elements(Streamname).Elements("Output").Elements("STR_MAIN").Elements("PRES").Elements("CISOLID").Value

    def STRM_CISOLID_Get_MoleFracPerCompound(self, Streamname, Compoundname):
        return self.STRM.Elements(Streamname).Elements("Output").Elements("MOLEFRAC").Elements("CISOLID").Elements(Compoundname).Value

    def STRM_CISOLID_Get_MassFracPerCompound(self, Streamname, Compoundname):
        return self.STRM.Elements(Streamname).Elements("Output").Elements("MASSFRAC").Elements("CISOLID").Elements(Compoundname).Value


    #DRYER INPUTS
    def BLK_DRYER_Set_Temperature(self, Blockname, Dryer_Temperature):
        self.BLK.Elements(Blockname).Elements("Input").Elements("TEMP").Value = Dryer_Temperature
    def BLK_DRYER_Set_Pressure(self, Blockname, Dryer_Pressure):
        self.BLK.Elements(Blockname).Elements("Input").Elements("PRES").Value = Dryer_Pressure
    def BLK_DRYER_Set_Type(self, Blockname:str, DryerType: Literal["SHORTCUT", "CONVECTIVE", "SPRAY", "CONTACT"]):
        self.BLK.Elements(Blockname).Elements("Input").Elements("DRYER_TYPE").Value = DryerType
    def BLK_DRYER_Set_Gasflowdirection(self, Blockname:str, Gasflowdir: Literal["COUNTERCURRE", "COCURRENT", "CROSSFLOW"]):
        self.BLK.Elements(Blockname).Elements("Input").Elements("FLOW_DIR").Value = Gasflowdir
    def BLK_DRYER_Set_Solidsflow(self, Blockname:str, Solidflow: Literal["MIXED", "PLUG-FLOW"]):
        self.BLK.Elements(Blockname).Elements("Input").Elements("SOLIDS_FLOW").Value = Solidflow
    def BLK_DRYER_Set_Length(self, Blockname, Length):
        self.BLK.Elements(Blockname).Elements("Input").Elements("LENGTH").Value = Length
    def BLK_DRYER_Set_Residencetime(self, Blockname, Restime):
        self.BLK.Elements(Blockname).Elements("Input").Elements("SOL_RES_TIME").Value = Restime
    def BLK_DRYER_Set_Solidmoisturebasis(self, Blockname:str, Moisturebasis: Literal["WET", "DRY"]):
        self.BLK.Elements(Blockname).Elements("Input").Elements("CURVE_SMC_B").Value = Moisturebasis
    def BLK_DRYER_Set_CriticalSMC(self, Blockname, CritSMC):
        self.BLK.Elements(Blockname).Elements("Input").Elements("CRIT_SMC").Value = CritSMC
    def BLK_DRYER_Set_EquilibriumSMC(self, Blockname, EquiSMC):
        self.BLK.Elements(Blockname).Elements("Input").Elements("EQUIL_SMC").Value = EquiSMC
    def BLK_DRYER_Set_NormDryCurveModel(self, Blockname:str, Curvemodel: Literal["FUNCTION", "DATA"]):
        self.BLK.Elements(Blockname).Elements("Input").Elements("CURVE_MODEL").Value = Curvemodel
    def BLK_DRYER_Set_Curveshapefactor(self, Blockname, Shapefactor):
        self.BLK.Elements(Blockname).Elements("Input").Elements("SHAPE_FACTOR").Value = Shapefactor
    def BLK_DRYER_Set_NormalizedMC(self, Blockname, Moisturecontent):
        self.BLK.Elements(Blockname).Elements("Input").Elements("SMC").Value = Moisturecontent
    def BLK_DRYER_Set_NormalizedDryingRate(self, Blockname, Dryingrate):
        self.BLK.Elements(Blockname).Elements("Input").Elements("RATE").Value = Dryingrate

    def BLK_DRYER_Get_DewPoint(self, Blockname):
        return self.BLK.Elements(Blockname).Elements("Output").Elements("EXDEW").Value


    #WASHER INPUTS
    def BLK_WASHER_Set_LiqtoSolMassRatio(self, Blockname, Ltsmassratio):
        self.BLK.Elements(Blockname).Elements("Input").Elements("LS_RATIO").Value = Ltsmassratio
    def BLK_WASHER_Set_MixingEfficiency(self, Blockname, Mixefficiency):
        self.BLK.Elements(Blockname).Elements("Input").Elements("MIX_EFF").Value = Mixefficiency

    #FILTER INPUTS

    def BLK_FILTER_Set_FracofSolidstoSolidsOutlet(self, Blockname, SolidFracOut):
        self.BLK.Elements(Blockname).Elements("Input").Elements("SOLID_SPLIT").Value = SolidFracOut
    def BLK_FILTER_Set_FracofLiqtoLiqOutlet(self, Blockname, LicFracOut):
        self.BLK.Elements(Blockname).Elements("Input").Elements("FLUID_SPLIT").Value = LicFracOut
    def BLK_FILTER_Set_LiqLoadofSolidsOutlet(self, Blockname, LiqLoadSolOut):
        self.BLK.Elements(Blockname).Elements("Input").Elements("FLUIDLOAD").Value = LiqLoadSolOut
    def BLK_FILTER_Set_SolLoadofLiqOutlet(self, Blockname, SolLoadLiqOut):
        self.BLK.Elements(Blockname).Elements("Input").Elements("SOLID_LOAD").Value = SolLoadLiqOut
    def BLK_FILTER_Set_Pressure(self, Blockname, Pressure):
        self.BLK.Elements(Blockname).Elements("Input").Elements("PRES").Value = Pressure
    def BLK_FILTER_Set_TemperatureChange(self, Blockname, DeltaT):
        self.BLK.Elements(Blockname).Elements("Input").Elements("DELT").Value = DeltaT

############################## Output for heater

    def BLK_HEATER_Get_HeatDuty(self, Blockname):
        return self.BLK.Elements(Blockname).Elements("Output").Elements("QCALC").Value
