...
parameters_dict = {
    'Electricity price': {'mean': 0.10, 'std': 0.0233},
    'Water price': {'mean': 0.10, 'std': 0.03},
    'Operator salary': {'mean': 3850, 'std': 255},
    'Interest rate': {'mean': 10, 'std': 1.67},
    'HCl buying price': {'mean': 112.03, 'std':16.67},
    'Project lifetime': {'mean': 20, 'std': 5},
    'Plant hours per year': {'mean': 8000, 'std': 250},
    'Fixed capital costs variance factor': {'mean': 1, 'std': 0.1},
    'Fixed production costs variance factor': {'mean': 1, 'std': 0.05},
    'Acid waste price': {'mean': 316, 'std': 40}
}
...
def full_calculation(parameters):
    ...
    plant_hours_per_year = parameters['Plant hours per year']
    ...
    irr = 100*calculate_irr(cash_flow) #*100 for decimal to percentage

    roi = roi_calculation(fixed_capital_costs_variance, cash_flow, project_lifetime)

    return levelized_cost_of_purification

############################################################################################End of Running the simulation

########################Monte Carlo
print(f"Now starting the Monte Carlo analysis with {number_of_iterations} iterations.")

parameter_results = {name: [] for name in parameters_dict.keys()}
lcop_results = []

for monte_carlo in range(number_of_iterations):
    sampled_parameters = {}

    for name, values in parameters_dict.items():
        sample = np.random.normal(values['mean'], values['std'])
        if sample < 0: #No neg params
            sample = 0
        ...
        sampled_parameters[name] = sample
        parameter_results[name].append(sample)
    lcop = full_calculation(sampled_parameters)

    #Remove inf values
    if not np.isinf(lcop) and not np.isnan(lcop):
        lcop_results.append(lcop)
    else:
        print(f"Skipping an iteration due to an invalid LCOP value: {lcop}")
...
parameters_units = {
    ...
    "Interest rate": "%",
    "HCl buying price": "$/tonne",
    "Project lifetime": "years",
    "Plant hours per year": "hours",
    "Fixed capital costs variance factor": "",
    "Fixed production costs variance factor": "",
    "Acid waste price": "$/tonne"
}

show_individual = False
num_plots = len(parameters_units)

columns = 3
rows = math.ceil(num_plots / columns)
figure_parameters, axes_parameters = plt.subplots(rows, columns, figsize=(15, 5*rows))
figure_parameters.suptitle('Probability Distributions of Input Parameters', fontsize=16)
plt.subplots_adjust(wspace=0.3, hspace=0.6)
axes_parameters_flat = axes_parameters.flatten()

for parameter, (naam, results) in enumerate(parameter_results.items()):
    mean_value = np.mean(results)
    std_value = np.std(results)
    axis = axes_parameters_flat[parameter]

    n, bins, patches = axis.hist(results, bins=50, density=True, edgecolor='black', alpha=0.7, color='lightblue')
    x = np.linspace(min(results), max(results), 200)
    pdf = norm.pdf(x, mean_value, std_value)
    axis.plot(x, pdf, 'r-', lw=2, label=fr'$\mu={mean_value:.2f}$, $\sigma={std_value:.2f}$')

    maximale_y = max(max(n), max(pdf))
    axis.set_ylim(top=maximale_y * 1.5)

    unit_tex = parameters_units.get(naam, "")
    if unit_tex:
        xlabel_text = f"{naam} [{unit_tex}]"
    else:
        xlabel_text = naam
    axis.set_xlabel(xlabel_text)
    axis.set_ylabel('Probability density')
    axis.grid(True, linestyle='--', alpha=0.6)
    axis.legend(loc='upper right', fontsize='small') #Pace the legend at the top right

    if show_individual:
        plt.figure(figsize=(8, 5))
        n_ind, bins_ind, patches_ind = plt.hist(results, bins=50, density=True, edgecolor='black', alpha=0.7, color='lightblue')
        plt.plot(x, pdf, 'r-', lw=2, label=fr'$\mu={mean_value:.2f}$, $\sigma={std_value:.2f}$')

        maximale_y_ind = max(max(n_ind), max(pdf))
        plt.ylim(top=maximale_y_ind * 1.5)
...
#This is to not show any empty plots
for number in range(num_plots, len(axes_parameters_flat)):
    figure_parameters.delaxes(axes_parameters_flat[number])


################## Plot of LCOP
mean_lcop = np.mean(lcop_results)
std_lcop = np.std(lcop_results)

plt.figure(figsize=(10, 6))
plt.hist(lcop_results, bins=100, density=True, edgecolor='black', alpha=0.7, color='lightcoral')
x_lcop = np.linspace(min(lcop_results), max(lcop_results), 200)
pdf_lcop = norm.pdf(x_lcop, mean_lcop, std_lcop)
plt.plot(x_lcop, pdf_lcop, 'r-', lw=2, label=fr'$\mu={mean_lcop:.2f}$, $\sigma={std_lcop:.2f}$')

plt.xlabel('LCOP [$/kg]', fontsize = 18)
