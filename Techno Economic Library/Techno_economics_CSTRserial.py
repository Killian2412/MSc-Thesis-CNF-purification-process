...
def capital_cost_calculation(cooler, cstr, separator, hydrocyclone, filter1, washer, filter2, solid_heater, dryer, air_filter, air_heater, steam_boiler, water_pump, air_cooler, cstr2, cstr3):
    c_total = cooler + cstr + separator + hydrocyclone + filter1 + washer + filter2 + solid_heater + dryer + air_filter + air_heater + steam_boiler + water_pump + air_cooler + cstr2 + cstr3
...
def opex_fixed_production_costs(isbl, vcop, solids_cooler_number, cstr_number, separator_number, hydrocyclone_number, filter1_number, washer_number, filter2_number, slurry_heater_number, dryer_number, operator_salary, cstr2_number, cstr3_number):

    # Labor costs
    operator_number = 5*(3 + solids_cooler_number + cstr_number + separator_number + hydrocyclone_number + filter1_number + washer_number + filter2_number + slurry_heater_number + dryer_number + cstr2_number + cstr3_number) #Towler2022: for fluids+solids continuous process there are 3 shift positions + 1 for every solids handling section. Then multiply by 5 for teams.
